...
/**
 * A MovingTransform3D is a Transform3D that has a location velocity and a angular rotation velocity for rotation around
 * the x, y, and z axes.
 */
public class MovingTransform extends Transform {
     ...
     */
    public MovingTransform(Transform v) {
        super(v);
        init();
    }
    ...
    public void update(long elapsedTime) {
        ...
        rotateAngle(velocityAngleX.getDistance(elapsedTime), velocityAngleY.getDistance(elapsedTime));
    }
     ...
     */
    public void moveTo(Vector2f destination, float speed) {
    ...
    public boolean isMoving() {
        return !velocityMovement.isStopped() && !velocity.equals(0, 0);
    }
    ...
    public boolean isMovingIgnoreY() {
        return !velocityMovement.isStopped() && velocity.x != 0;
    }
    ...
    /**
     * Gets the velocity vector. If the velocity vector is modified directly, call setVelocity() to ensure the
    ...
    /**
     * Sets the velocity. The velocity is automatically set to zero after the specified amount of time has elapsed.
     * If the specified time is FOREVER, then the velocity is never automatically set to zero.
     */
    public void setVelocity(Vector2f v, long time) {
        if (velocity != v) {
            velocity.set(v);
        }
        if (v.x == 0 && v.y == 0) {
    ...
    /**
     * Adds the specified velocity to the current velocity. If this MovingTransform3D is currently moving, it's time
     * remaining is not changed. Otherwise, the time remaining is set to FOREVER.
     */
    ...
    /**
     * Ensures the specified angle is with -pi and pi. Returns the angle, corrected if it is not within these
     * bounds.
     */
    ...
    /**
     * The Movement class contains a speed and an amount of time to continue that speed.
     */
    protected static class Movement {
        ...
        /**
         * Gets the distance traveled in the specified amount of time in milliseconds.
         *
         * @param elapsedTime
         * @return
         */
        public float getDistance(long elapsedTime) {
            if (remainingTime == 0) {
                ...
                elapsedTime = (long) Math.min(elapsedTime, remainingTime);
